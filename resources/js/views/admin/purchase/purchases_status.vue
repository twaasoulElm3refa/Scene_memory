    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1">View and manage purchases by status</p>
          </div>
        ...
        <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-blue-900">${{ stats.totalAmount }}</p>
            </div>
      ...
      <div v-else class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span
                    :class="{
                      ...
                      'bg-gray-100 text-gray-800': purchase.status === 'refunded',
                    }"
        ...
        <div v-if="purchases.length === 0" class="text-center py-12">
          ...
          <p class="text-gray-500">No purchases found with status "{{ selectedStatus }}".</p>
        </div>
...
const selectedStatus = ref(route.params.status || "completed");
...
// Stats for all statuses (would ideally come from API, but we calculate from current data)
const stats = computed(() => {
  // For now, we calculate from current purchases
  // In production, you might want to fetch these from a separate endpoint
  const completed = purchases.value.filter(p => p.status === "completed").length;
  const pending = purchases.value.filter(p => p.status === "pending").length;
  const failed = purchases.value.filter(p => p.status === "failed").length;
  const totalAmount = purchases.value.reduce((sum, p) => sum + parseFloat(p.amount || 0), 0).toFixed(2);

  return {
    completed: pagination.value.total && selectedStatus.value === "completed" ? pagination.value.total : completed,
    pending: selectedStatus.value === "pending" ? pagination.value.total : pending,
    failed: selectedStatus.value === "failed" ? pagination.value.total : failed,
    totalAmount,
  };
});
...
const fetchPurchases = async (page = 1) => {
  ...
  try {
    const response = await PurchaseService.getByStatus(selectedStatus.value, page);
...
const changeStatus = () => {
  router.push(`/admin/purchases/${selectedStatus.value}/status`);
  fetchPurchases(1);
};
...
onMounted(() => {
  if (route.params.status) {
    selectedStatus.value = route.params.status;
  }
  fetchPurchases();
});
</script>
