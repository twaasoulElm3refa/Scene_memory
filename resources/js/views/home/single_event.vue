    ...
    <div class="min-h-screen bg-gray-50">
        ...
        <div v-else>
            ...
            <div class="relative">
                ...
                >
                    <div class="w-24 h-24 md:w-32 md:h-32 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-2xl">
                ...
                <div class="absolute top-6 left-6 md:left-10 flex flex-wrap gap-3 z-20">
                    <span class="bg-blue-100/90 backdrop-blur-sm text-blue-900 px-5 py-2 rounded-full text-base font-bold shadow-lg uppercase tracking-wider">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-0">
                        ...
                        <div class="lg:col-span-2 p-8 md:p-12 lg:p-16">
                            ...
                            <div class="flex items-center justify-between mb-6">
                                ...
                                >
                                    <svg class="w-7 h-7 transition-transform" :class="{ 'scale-110': isLiked }" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <!-- Like Error -->
                            <p v-if="likeError" class="mb-4 text-red-600 bg-red-50 border border-red-200 px-4 py-2 rounded-lg text-sm">
                                {{ likeError }}
                            </p>
                            ...
                            <div class="mb-12">
                                <div class="flex items-center justify-between mb-6">
                                    <h2 class="text-2xl md:text-3xl font-bold text-gray-900 border-b border-gray-200 pb-4">
                                ...
                                <div v-if="hasMedia" class="grid grid-cols-2 md:grid-cols-3 gap-2">
                                    <div
                                    ...
                                    >
                                        <img
                                            ...
                                            loading="lazy"
                                        />
                                        <div v-else-if="media.video || (media.url && isVideoUrl(media.url))" class="relative w-full h-full bg-black">
                                            <video :src="media.video || media.url" class="w-full h-full object-cover" muted loop playsinline preload="metadata"></video>
                                ...
                                <div v-else class="text-center py-12 text-gray-500">
                                    {{ $t('event.no_media_yet') || 'لا توجد وسائط بعد' }}
                                </div>
                            </div>
                            ...
                            <div class="comments-section mt-12">
                                <h2 class="text-2xl md:text-3xl font-bold text-gray-900 mb-8 border-b border-gray-200 pb-4">
                                ...
                                >
                                    ...
                                    <p v-if="deleteCommentErrors[comment.id]" class="mb-2 text-red-600 bg-red-50 border border-red-200 px-3 py-1.5 rounded-lg text-xs">
                                    ...
                                    <p v-if="reactionErrors[comment.id]" class="mt-2 text-red-600 bg-red-50 border border-red-200 px-3 py-1.5 rounded-lg text-xs">
                                    ...
                                    <div v-if="isAuthenticated && replyingTo === comment.id" class="mt-6 pl-12">
                                        <form @submit.prevent="addReply(comment.id)" class="space-y-4">
                                            ...
                                            <!-- Reply Error -->
                                            <p v-if="replyErrors[comment.id]" class="text-red-600 bg-red-50 border border-red-200 px-3 py-2 rounded-lg text-sm">
                                                {{ replyErrors[comment.id] }}
                                            </p>

                                            <div class="flex justify-end gap-3">
                                                <button type="button" @click="cancelReply(comment.id)" class="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">
                                                    {{ $t('cancel') || 'إلغاء' }}
                                                </button>
                                                <button
                                                    type="submit"
                                    ...
                                    <!-- Replies -->
                                    <div v-if="comment.replies?.length > 0" class="mt-6 pl-12 border-l-2 border-indigo-200 space-y-6">
                                        <div v-for="reply in comment.replies" :key="reply.id" class="bg-white p-4 rounded-lg shadow-sm">
                                            <div class="flex items-start gap-3">
                                                ...
                                                <div class="flex-1">
                                                    <div class="flex items-center gap-2 mb-1">
                                                        <p class="font-medium text-gray-900">{{ reply.user?.name || $t('event.visitor') }}</p>
                                ...
                                <!-- Add Comment Form -->
                                <div v-if="isAuthenticated" class="comment-form mt-12 bg-white p-8 rounded-xl shadow-md border border-gray-100">
                                    ...
                                    <form @submit.prevent="addComment">
                                        ...
                                        <p v-if="commentError" class="mt-3 text-red-600 bg-red-50 border border-red-200 px-4 py-2 rounded-lg text-sm">
                                            {{ commentError }}
                                        </p>
                                        <p v-if="commentSuccess" class="mt-3 text-green-600 bg-green-50 border border-green-200 px-4 py-2 rounded-lg text-sm">
                                ...
                                <div v-else class="text-center py-12 bg-gray-50 rounded-xl">
                                    ...
                                    <RouterLink to="/auth" class="inline-block px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                        ...
                        <div class="bg-gray-50/70 p-8 md:p-12 lg:p-16 border-t lg:border-t-0 lg:border-l border-gray-200 lg:sticky lg:top-0 lg:h-screen lg:overflow-y-auto">
                            <div class="space-y-10">
                                ...
                                <div class="pt-8 border-t border-gray-200 space-y-4">
                                    ...
                                    <p v-if="wishlistError" class="text-red-600 text-center text-sm mt-2 bg-red-50 border border-red-200 p-3 rounded-lg">
                                        {{ wishlistError }}
                                    </p>
                                    <p v-if="wishlistSuccess" class="text-green-600 text-center text-sm mt-2 bg-green-50 border border-green-200 p-3 rounded-lg">
            ...
            <!-- Lightbox -->
            <div v-if="lightboxOpen" class="fixed inset-0 bg-black/95 z-50 flex items-center justify-center" @click="lightboxOpen = false">
                <div class="relative max-w-[95vw] max-h-[95vh]">
                    ...
                    <video
                        ...
                        @click.stop
                    ></video>
                    <button class="absolute top-4 right-4 text-white text-5xl font-bold drop-shadow-lg" @click="lightboxOpen = false">×</button>
                </div>
            </div>

            <!-- Upload Modal -->
            <div v-if="showUploadModal" class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" @click.self="showUploadModal = false">
                <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="p-6">
                        <div @dragover.prevent @drop.prevent="handleDrop" class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-indigo-500 transition-colors">
                            <input type="file" multiple accept="image/*,video/mp4,video/webm,video/quicktime" class="hidden" id="mediaUploadInput" @change="handleFileChange" />
                        ...
                        <div v-if="selectedFiles.length > 0" class="mt-6">
                            ...
                            <ul class="space-y-2 max-h-48 overflow-y-auto pr-2">
                                <li v-for="(file, idx) in selectedFiles" :key="idx" class="flex items-center justify-between bg-gray-50 p-3 rounded-lg">
                                    <div class="flex items-center gap-3 truncate">
                                        <span class="text-gray-600">{{ file.name }}</span>
                                        <span class="text-xs text-gray-500">({{ formatFileSize(file.size) }})</span>
                                    </div>
                                    <button @click="removeFile(idx)" class="text-red-600 hover:text-red-800 text-sm">حذف</button>
                                </li>
                            </ul>
                        </div>

                        <p v-if="uploadError" class="mt-4 text-red-600 bg-red-50 border border-red-200 p-3 rounded-lg">{{ uploadError }}</p>
                        <p v-if="uploadSuccess" class="mt-4 text-green-600 bg-green-50 border border-green-200 p-3 rounded-lg">{{ uploadSuccess }}</p>
                    </div>
                    <div class="p-6 border-t flex gap-3 justify-end">
                        <button @click="showUploadModal = false" class="px-6 py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50">إلغاء</button>
                        <button
                            @click="uploadFiles"
                ...
                <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10">
                    ...
                    <template v-else>
                        <div class="flex items-center justify-between mb-5">
                            ...
                            <button @click="closeReportModal" class="w-7 h-7 flex items-center justify-center rounded-full text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                        ...
                        <p v-if="reportError" class="text-xs text-red-500 mb-3 bg-red-50 border border-red-200 px-3 py-2 rounded-lg">
                            ⚠️ {{ reportError }}
                        </p>

                        <div class="flex gap-2 justify-end">
                            <button @click="closeReportModal" class="px-4 py-2 rounded-xl border border-gray-200 text-sm text-gray-600 hover:bg-gray-50 transition">
                                إلغاء
                            </button>
                            <button
                                @click="submitReport"
