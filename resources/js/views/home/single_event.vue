<template>
    <div class="min-h-screen bg-gray-50">
        ...
        <div v-else>
            <div class="relative">
                <!-- Hero Media -->
                <component :is="heroMediaComponent" v-if="heroMedia" :src="heroMedia.url"
                ...
                <div v-if="heroMedia && (heroMedia.video || isVideoUrl(heroMedia.url))"
                        ...
                        <div class="lg:col-span-2 p-8 md:p-12 lg:p-16">
                            ...
                            <div class="mb-12">
                                ...
                                <div v-if="hasMedia" class="grid grid-cols-2 md:grid-cols-3 gap-2">
                                    <div v-for="(media, index) in event.images" :key="media.id || index"
                                        ...
                                        <img v-if="!media.video && media.url && !isVideoUrl(media.url)" :src="media.url"
                                            class="w-full h-full object-cover" loading="lazy" />

                                        <div v-else-if="media.video || (media.url && isVideoUrl(media.url))"
                                            class="relative w-full h-full bg-black">
                                            <video :src="media.video || media.url" class="w-full h-full object-cover"
                            ...
                            <div class="comments-section mt-12">
                                <h2
                                    ...
                                    {{ $t('event.comments_title') }} ({{ event.comments_count || event.comments?.length
                                    || 0 }})
                                </h2>
                                ...
                                <div v-for="comment in event.comments" :key="comment.id"
                                    class="comment-box mb-10 bg-gray-50 p-6 rounded-xl shadow-sm">

                                    <div class="comment-content flex justify-between items-start">
                                        <div class="comment-text flex-1">
                                            ...
                                            <p class="comment-body text-gray-700 leading-relaxed mt-2">
                                        ...
                                        <form @submit.prevent="addReply(comment.id)" class="space-y-4">
                                            ...
                                            <div class="flex justify-end gap-3">
                                                ...
                                                <button type="submit"
                                                    ...
                                                    <span v-if="replyLoading[comment.id]">{{ $t('sending') || 'جار الاسرال ...' }}</span>
                                                ...
                                                <div class="flex-1">
                                                    <div class="flex items-center gap-2 mb-1">
                                                        <p class="font-medium text-gray-900">
                                                            {{ reply.user?.name || $t('event.visitor') }}
                                                        </p>
                                ...
                                <!-- نموذج إضافة تعليق رئيسي (يظهر فقط للمسجلين) -->
                                <div v-if="isAuthenticated"
                                    ...
                                    <form @submit.prevent="addComment">
                                        ...
                                        <p v-if="commentError" class="mt-3 text-red-600 text-sm">
                                            {{ commentError }}
                                        </p>
                                        <p v-if="commentSuccess" class="mt-3 text-green-600 text-sm">
                                            {{ $t('event.comment_added_success') || 'تم إضافة التعليق بنجاح' }}
                                        </p>
                                    </form>
                                </div>

                                <!-- رسالة تسجيل الدخول إذا لم يكن مسجلاً -->
                                <div v-else class="text-center py-12 bg-gray-50 rounded-xl">
                                    <p class="text-lg text-gray-700 mb-4">
                                        {{ $t('event.login_to_comment') || 'يجب عليك تسجيل الدخول لإضافة تعليق أو الرد'
                                        }}
                                    </p>
                                    <RouterLink to="/login"
                            ...
                            <div class="space-y-10">
                                ...
                                <div class="pt-8 border-t border-gray-200 space-y-4">
                                    <button @click="addToWishlist" :disabled="wishlistLoading || isInWishlist"
                                        ...
                                        :class="{
                                            'opacity-70 cursor-not-allowed': wishlistLoading || isInWishlist,
                                        }">
                                        <span v-if="wishlistLoading" class="animate-pulse">{{
                                            $t('event.adding_to_wishlist')
                                            }}</span>
                                        <template v-else>
                                            ♥
                                            {{
                                                isInWishlist
                                                    ? $t('event.already_in_wishlist')
                ...
                <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                    <div class="p-6 border-b">
                        ...
                        <p class="text-gray-600 mt-1 text-sm">
                            الحد الأقصى لكل ملف: 100 ميجابايت
                        </p>
                    </div>
                    ...
                    <div class="p-6">
                        <div @dragover.prevent @drop.prevent="handleDrop"
                            ...
                            <label for="mediaUploadInput" class="cursor-pointer">
                                ...
                                <p class="text-sm text-gray-500 mt-2">
                                    صور وفيديوهات (max 100MB لكل ملف)
                                </p>
                            </label>
                        </div>
                        ...
                        <div v-if="selectedFiles.length > 0" class="mt-6">
                            ...
                            <ul class="space-y-2 max-h-48 overflow-y-auto pr-2">
                                <li v-for="(file, idx) in selectedFiles" :key="idx"
                                    ...
                                    <button @click="removeFile(idx)" class="text-red-600 hover:text-red-800 text-sm">
                                        حذف
                                    </button>
                                </li>
                            </ul>
                        ...
                        <p v-if="uploadError" class="mt-4 text-red-600 bg-red-50 p-3 rounded-lg">
                            {{ uploadError }}
                        </p>
                        <p v-if="uploadSuccess" class="mt-4 text-green-600 bg-green-50 p-3 rounded-lg">
                            {{ uploadSuccess }}
                        </p>
                    </div>
...
const addToWishlist = async () => {
    ...
    try {
        ...
        console.error("خطأ أثناء إضافة إلى المفضلة:", err);
        const errorMsg =
            err.response?.data?.message ||
            err.response?.data?.error ||
            "حدث خطأ أثناء إضافة الحدث إلى المفضلة";
...
const currentMedia = computed(
    () => event.value?.images?.[lightboxIndex.value] || null
);
...
const addComment = async () => {
    ...
    try {
        ...
        commentError.value =
            err.response?.data?.message || "حدث خطأ أثناء إرسال التعليق";
    } finally {
...
const addValidFiles = (files) => {
    ...
    files.forEach((file) => {
        if (file.size > MAX_SIZE) {
            alert(`الملف ${file.name} كبير جدًا (أكبر من 100 ميجا)`);
            return;
        }
...
const uploadFiles = async () => {
    if (!event.value?.id || selectedFiles.value.length === 0) return;
    ...
    try {
        ...
        if (res.data.status === "success") {
            if (res.data.data?.media && Array.isArray(res.data.data.media)) {
                res.data.data.media.forEach((newMedia) => {
                    event.value.images.push(newMedia);
                });
            } else if (res.data.data?.media) {
                event.value.images.push(res.data.data.media);
            }
