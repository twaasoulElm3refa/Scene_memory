    ...
    <div class="card">
      ...
      <p class="hint">
        {{ timedOut
          ...
          : "You may close this page; confirmation continues in the background." }}
      </p>
    </div>
  </div>
</template>
...
onMounted(async () => {
  if (!orderId) {
    router.replace(`/${lang}/deposit/failed`);
    return;
  }

  const result = await pollOrderStatus({
    signal: controller.signal,
    fetchStatus: async (signal) => (await PaymentService.walletOrderStatus(orderId, signal)).data.status,
  });
  if (result.outcome === "completed") {
    clearIdempotencyKey("wallet_deposit:paypal");
    router.replace(`/${lang}/deposit/success`);
...
.spinner { width: 56px; height: 56px; border: 5px solid #e5e7eb; border-top-color: #16a34a; border-radius: 50%; margin: 0 auto 24px; animation: spin .9s linear infinite; }
